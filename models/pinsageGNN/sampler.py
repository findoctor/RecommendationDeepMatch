...
def postive_sampler(g, heads, utype, itype, \
                     user2item_type, item2user_type, e_fea_name='rating', sampler_type='positive'):
    """
    Generating postive or negative samples from heads ID
    Criterion: 

     Parameters
    ----------
    heads: tensor list of heads ID
    sampler_type: pos or negative sampling
    """
    nbrs = []  
    for head in heads:
        tar_item = -1
        min_score_diff = 0xfffffff   # score diff of (head_item, neighbor_item)
        reviewed_users = g.successors(head, etype=item2user_type)
        for candidate_user in reviewed_users:
            # Edge ID, end point (user_candidate, head)
            c2head_ID = g.edge_ids([candidate_user], [head], etype=(utype, user2item_type, itype))  
            base_score = g.edges[user2item_type].data[e_fea_name][c2head_ID]
            reviewed_items = list(int(item) for item in g.successors(candidate_user, etype=user2item_type) )
            reviewed_items.remove(head)
            # user review scores
            if len(reviewed_items) < 2: continue
            review_scores = g.edges[user2item_type].data[e_fea_name][torch.LongTensor(reviewed_items)]
            # diff score list
            diff_scores = review_scores.numpy()-float(base_score)
            cur_min_score_diff = np.min(diff_scores)
            if cur_min_score_diff < min_score_diff:
                min_score_diff = cur_min_score_diff
                tar_item = np.argmin(diff_scores)
        nbrs.append(tar_item)
    return torch.LongTensor(nbrs)
...
class ItemToItemBatchSampler(IterableDataset):
    ...
    def __iter__(self):
        while True:
            ...
            tails = postive_sampler(self.g, heads, self.user_type, self.item_type, \
                                    self.user_to_item_etype, self.item_to_user_etype)
...
class NeighborSampler(object):
    ...
    def sample_blocks(self, seeds, heads=None, tails=None, neg_tails=None):
        ...
        for sampler in self.samplers:
            ...
            seeds = block.srcdata[dgl.NID]  # srcdata 也包含了seeds的邻居(想要更新seeds 那么在上一层就必须先更新seeds的邻居)
...
def assign_simple_node_features(ndata, g, ntype, assign_id=False):
    ...
    """
    # ndata: blocks[0].srcdata 是dict key:_ID  values: 原图中的ID list
